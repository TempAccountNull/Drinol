...
void halo3::hooks::init_function_calls()
{
	// Declaring these inline does not work! So this function has been made to properly assign these vars.
	game_time_set_rate_scale_direct_og = reinterpret_cast<game_time_set_rate_scale_direct>(reinterpret_cast<char*>(halo3::offsets::game_time_set_rate_scale_direct));
}
...
{
	if (halo3::hooks::change_speed)
	{
		game_time_set_rate_scale_direct_og(ui::game_speed);

		halo3::hooks::change_speed = false;
	}
