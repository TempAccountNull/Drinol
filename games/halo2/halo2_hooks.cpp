...
void halo2::hooks::init_function_calls()
{
	// Declaring these inline does not work! So this function has been made to properly assign these vars.
	game_time_set_rate_scale_direct_og = reinterpret_cast<game_time_set_rate_scale_direct>(reinterpret_cast<char*>(halo2::offsets::game_time_set_rate_scale_direct));
}
...
{
	if (halo2::hooks::change_speed)
	{
		game_time_set_rate_scale_direct_og(ui::game_speed);

		halo2::hooks::change_speed = false;
	}

	return game_update_og(a1, a2);
}

inline void halo2::hooks::hook_game_update()
{
...
inline void halo2::hooks::unhook_game_update()
{
	MH_DisableHook(game_update_pointer);
	MH_RemoveHook(game_update_pointer);
}

void halo2::hooks::init_hooks()
{
	hook_game_update();
}

void halo2::hooks::deinit_hooks()
{
	unhook_game_update();
}

void halo2::hooks::reinit_hooks()
{
	deinit_hooks();
	Sleep(1000);
	init_hooks();
